...
Companion skills already in Issuebridge stay in-tree until Issuebridge chooses otherwise. The named library is **[Matt Pocock's skills](https://github.com/mattpocock/skills)**; Agent-kit does not own Issuebridge's copies. See [README](../README.md) and [Adopt](adopt.md).

---

## Keeps (Consumer-only)

- Threat pack (threat model + findings ledger)
...
3. Keep kill switches, allowlists, labels, `CLAUDE_CODE_OAUTH_TOKEN`, the advisories token, optional Dependabot alerts token, optional email (SMTP; Resend works). Kill switch variables: `CLAUDE_PIPELINE_ENABLED`, `CLAUDE_SECURITY_AUDIT_ENABLED`.
