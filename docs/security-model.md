...
- The implementer loads the same hook. Read confinement covers the Read/Grep/Glob tools only: an `extra_allowed_tools` entry that runs repository code (a test runner, a package manager, a formatter that loads plugins) lets the implementer read anything the runner user can, including the credentials in its own environment, and act with the Claude GitHub App token. Grant those only for repositories whose issue authors you trust.
