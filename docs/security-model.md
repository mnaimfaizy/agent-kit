...
- A PreToolUse hook in `.github/agent-runtime/` denies Read/Grep/Glob outside the workspace, judged on the path with symlinks resolved, and for Glob on the pattern too: an absolute pattern, a `..` segment, or a literal prefix that resolves outside is refused. Review and audit restore that hook from the pull request base. Before the agent starts, each job copies the hook and its settings read-only to `$RUNNER_TEMP`, outside the workspace, and runs that copy; a digest check after the agent fails the job before anything is published.
